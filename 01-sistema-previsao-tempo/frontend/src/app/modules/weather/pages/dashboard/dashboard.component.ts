...
@Component({
  ...
  template: `
    <div class="dashboard">
      <div class="page-header">
        <h1>Dashboard</h1>
        <p class="page-subtitle">Consulte a previsão do tempo em qualquer cidade</p>
      </div>
  ...
  styles: [`
    ...
    .search-input-wrapper input:focus {
      outline: none;
      border-color: #4A90D9;
      box-shadow: 0 0 0 3px rgba(74, 144, 217, 0.1), 0 4px 14px rgba(26, 35, 50, 0.06);
      background: white;
    }
    ...
    .chip:hover {
      ...
      box-shadow: 0 3px 10px rgba(74, 144, 217, 0.1);
    }
...
export class DashboardComponent implements OnInit {
  ...
  searchWeather(): void {
    ...
    this.weatherService.getCurrentWeather(this.searchCity, 'pt').subscribe({
      next: (response) => {
        if (response.success) {
      ...
      error: (err) => {
        this.error = err.error?.message || 'Erro ao buscar previsão';
        this.loading = false;
      }
    });
  }
