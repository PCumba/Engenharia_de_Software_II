  ...
  template: `
    <div class="search-page">
      <div class="page-header">
        <h1>{{ i18n.t('weather.searchTitle') }}</h1>
        <p class="page-subtitle">{{ i18n.t('weather.searchSubtitle') }}</p>
      </div>
      <div class="search-form">
        <div class="input-group">
          <div class="input-wrapper">
            <span class="input-icon">🔍</span>
            <input type="text" [(ngModel)]="city" [placeholder]="i18n.t('weather.searchPlaceholder')" (keyup.enter)="search()">
          </div>
          <button (click)="search()" [disabled]="loading" class="btn-primary">{{ loading ? i18n.t('weather.searching') : i18n.t('weather.search') }}</button>
          <button (click)="addToFavorites()" *ngIf="currentWeather" class="btn-favorite">⭐ {{ i18n.t('weather.addFavorite') }}</button>
        </div>
      ...
      <div class="results" *ngIf="currentWeather">
        ...
        <div class="export-row">
          <button (click)="exportCSV()" class="btn-export">📥 {{ i18n.t('weather.exportCSV') }}</button>
          <button (click)="exportPDF()" class="btn-export btn-pdf">📄 {{ i18n.t('weather.exportPDF') }}</button>
        </div>
      </div>
      <div class="loading-state" *ngIf="loading">
        <div class="spinner"></div>
        <p>{{ i18n.t('weather.loading') }}</p>
      </div>
    </div>
  `,
  styles: [`
    ...
    .page-header h1 { font-size: 2rem; font-weight: 800; letter-spacing: -0.03em; color: var(--text-primary); margin-bottom: 0.25rem; }
    ...
    .export-row { display: flex; gap: 0.75rem; margin-top: 1.25rem; }
    .btn-export { display: inline-flex; align-items: center; gap: 0.4rem; padding: 0.7rem 1.25rem; background: rgba(255,255,255,0.9); border: 1.5px solid rgba(74,144,217,0.15); border-radius: 12px; color: var(--text-secondary); font-weight: 600; font-size: 0.875rem; cursor: pointer; transition: all 250ms ease; }
    .btn-export:hover { border-color: #4A90D9; color: #4A90D9; background: rgba(74,144,217,0.04); transform: translateY(-1px); }
    .btn-pdf { border-color: rgba(229,62,62,0.15); }
    .btn-pdf:hover { border-color: #E53E3E; color: #E53E3E; background: rgba(229,62,62,0.04); }
...
export class SearchComponent {
  ...
  search(): void {
    ...
    this.weatherService.getCurrentWeather(this.city, this.i18n.getCurrentLang()).subscribe({
  ...
  addToFavorites(): void {
    ...
    this.weatherService.addFavorite(this.currentWeather.city, this.currentWeather.country).subscribe({
      next: (response) => { if (response.success) { alert(this.i18n.t('weather.addedFavorite')); } },
  ...
  exportCSV(): void {
    if (this.currentWeather) {
      this.exportService.exportCSV([this.currentWeather], 'weather_search');
    }
  }

  exportPDF(): void {
    if (this.currentWeather) {
      this.exportService.exportPDF([this.currentWeather], 'weather_search');
    }
  }
}
