...
@Component({
  ...
  template: `
    <div class="forecast-card" *ngIf="forecasts && forecasts.length > 0">
      <h3>{{ i18n.t('weather.fiveDayForecast') }}</h3>
  ...
  styles: [`
    ...
    .forecast-card h3 { margin: 0 0 1.25rem 0; color: var(--text-primary); font-size: 1.1rem; font-weight: 700; }
    ...
    .date-time { font-size: 0.8rem; color: var(--text-muted); line-height: 1.3; }
    .date-time strong { color: var(--text-secondary); font-weight: 700; }
    .forecast-temp { font-size: 1.15rem; font-weight: 800; color: #4A90D9; }
    .forecast-desc { text-transform: capitalize; font-size: 0.8rem; color: var(--text-secondary); font-weight: 500; }
    .forecast-meta { display: flex; gap: 0.75rem; font-size: 0.75rem; color: var(--text-muted); grid-column: 1 / -1; }
...
export class ForecastComponent implements OnInit {
  ...
  loadForecast(): void {
    this.loading = true;
    this.weatherService.getFiveDayForecast(this.city, this.i18n.getCurrentLang()).subscribe({
      next: (response) => { if (response.success) { this.forecasts = response.data.forecasts; } this.loading = false; },
      error: (err) => { console.error('Error loading forecast:', err); this.loading = false; }
    });
  }
}
