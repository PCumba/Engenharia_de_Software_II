...
@Component({
    ...
    <div class="login-container">
      <div class="login-card">
        <div class="card-header">
          ...
          <p class="subtitle">{{ i18n.t('auth.welcome') }}</p>
        </div>
        ...
        <form [formGroup]="form" (ngSubmit)="onLogin()">
          ...
          <button type="submit" class="btn-submit" [disabled]="form.invalid || loading">
            {{ loading ? i18n.t('auth.logging') : i18n.t('auth.login') }}
          </button>
        ...
        <p class="forgot-link">
          <a routerLink="/auth/forgot-password">{{ i18n.t('auth.forgotPassword') }}</a>
        </p>
        <p class="register-link">
          {{ i18n.t('auth.noAccount') }} <a routerLink="/auth/register">{{ i18n.t('auth.registerHere') }}</a>
        </p>
      </div>
    </div>
  `,
  styles: [`
    ...
    .forgot-link a { color: #4A90D9; text-decoration: none; font-weight: 600; font-size: 0.875rem; }
    .register-link { text-align: center; margin-top: 0.75rem; color: #8896A6; font-size: 0.875rem; }
    .register-link a { color: #4A90D9; text-decoration: none; font-weight: 700; }
    .register-link a:hover, .forgot-link a:hover { color: #2E6AB3; }
  `]
})
