...
@Component({
    ...
    <div class="register-container">
      <div class="register-card">
        <div class="card-header">
          ...
          <p class="subtitle">{{ i18n.t('auth.registerWelcome') }}</p>
        </div>
        ...
        <form [formGroup]="form" (ngSubmit)="onRegister()">
          ...
          <div class="form-group">
            ...
            <input type="email" formControlName="email" placeholder="seu@email.com" required>
        ...
        <p class="login-link">
          {{ i18n.t('auth.hasAccount') }} <a routerLink="/auth/login">{{ i18n.t('auth.loginHere') }}</a>
        </p>
      </div>
    </div>
  `,
  styles: [`
    ...
    .register-card {
      ...
      padding: 2.5rem; border-radius: 20px; border: 1px solid rgba(74, 144, 217, 0.1);
      box-shadow: 0 20px 60px rgba(26, 35, 50, 0.08); width: 100%; max-width: 420px;
    ...
    h1 { font-size: 1.75rem; font-weight: 800; background: linear-gradient(135deg, #4A90D9 0%, #2E6AB3 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; margin-bottom: 0.4rem; }
    .subtitle { color: #8896A6; font-size: 0.9rem; margin: 0; }
    .form-group { margin-bottom: 1.25rem; }
    label { display: block; margin-bottom: 0.4rem; color: #4A5568; font-size: 0.8125rem; font-weight: 600; text-transform: uppercase; }
    ...
    .error-box { color: #E53E3E; font-size: 0.875rem; text-align: center; padding: 0.75rem 1rem; background: rgba(229, 62, 62, 0.06); border: 1px solid rgba(229, 62, 62, 0.15); border-radius: 10px; margin-top: 1rem; }
    .login-link { text-align: center; margin-top: 1.75rem; color: #8896A6; font-size: 0.875rem; }
    .login-link a { color: #4A90D9; text-decoration: none; font-weight: 700; }
...
export class RegisterComponent {
  ...
  onRegister(): void {
    ...
    this.authService.register(email, password, name).subscribe({
      next: (response) => {
        if (response.success) { this.router.navigate(['/auth/login']); }
        this.loading = false;
      },
      error: (err) => {
