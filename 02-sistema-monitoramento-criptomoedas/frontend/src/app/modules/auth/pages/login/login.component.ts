...
@Component({
  ...
  template: `
    <div class="auth-container">
      <div class="auth-card">
        <div class="card-header"><span class="brand-icon">◈</span><h1>CryptoMonitor</h1><p class="subtitle">Faça login na sua conta</p></div>
...
export class LoginComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    this.authService.login(this.loginForm.value).subscribe({ next: () => { this.router.navigate(['/crypto/dashboard']); this.loading = false; }, error: (err) => { this.error = err.error?.message || 'Erro ao fazer login'; this.loading = false; } });
  }
}
